...
_validate_deployment() {
    ss-set statecustom "Validating deployed services..."
...
_gh_scripts_url() {
    ...
    echo $_GH_PROJECT_URL/$branch/install
}

_install_with_refconf() {
    ss-set statecustom "Installing SlipStream WITH reference configuration..."
    curl -k -sSfL -o /tmp/ss-install-ref-conf.sh \
